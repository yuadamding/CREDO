...
class CounterfactualResult:
    ...
    def terminal_mass_diff(self) -> float:
        logw_p = self.rollout_perturb.terminal_logw
        logw_c = self.rollout_control.terminal_logw
        mass_p = float(torch.exp(torch.logsumexp(logw_p, 0)).item())
        mass_c = float(torch.exp(torch.logsumexp(logw_c, 0)).item())
...
class CounterfactualEngine:
    ...
    def run(
        ...
        """
        results = []
        ctrl_ids = list(self.model.control_ids)
        ...
        for pid in perturbation_ids:
            ...
            rollout_p = self.simulator.rollout(
                z0=z0p, logw0=lw0p, model=self.model, log_m0=lm0p)

            # --- Control rollout (use same initial conditions) ---
            ctrl_pid = ctrl_ids[0]
            if ctrl_pid not in endpoint.initial:
                # Use the perturbation initial condition for control
                z0c, lw0c, lm0c = z0p.clone(), lw0p.clone(), lm0p.clone()
            else:
                z0c, lw0c, lm0c = initialise_particles(
                    endpoint, [ctrl_pid], self.n_particles, self.device, seed=seed)
            ...
            with _zero_embedding_context(self.model, pid):
                rollout_c = self.simulator.rollout(
                    z0=z0c, logw0=lw0c, model=self.model, log_m0=lm0c)
