...
"""Single-screen learned particle simulator."""
...
from camfnd.models.coeff_nets import ControlAnchoredStage1Model
...
class LearnedStage1Simulator:
    def __init__(self, endpoint_problem: EndpointProblem, config: LearnedSimulatorConfig) -> None:
        ...
        if int(latent_dim) != 1:
            raise NotImplementedError("Step 3 reference implementation currently supports the 1D Stage-I benchmark only.")

    @classmethod
    def from_dataset(cls, dataset: PerturbSeqDynamicsData, *, config: LearnedSimulatorConfig) -> "LearnedStage1Simulator":
        dataset.validate()
    ...
    def run(self, model: ControlAnchoredStage1Model) -> LearnedSimulationResult:
        model.eval()
        dt = self.dt
        ...
        if self.config.store_history:
            history = {key: [] for key in self.problem.keys}
        ...
        for step_idx in range(self.config.n_steps):
            ...
            for key in self.problem.keys:
                state = particles[key]
                coeffs = model.coefficients(state.z, t_scalar, state.perturbation_id)
        ...
        return LearnedSimulationResult(
            config=self.config,
...
SingleScreenSimulationResult = LearnedSimulationResult
LearnedSingleScreenSimulator = LearnedStage1Simulator
