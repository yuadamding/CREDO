...
\section{Explicit introduction to the weighted SDE}
...
\subsection{From a representative trajectory to a finite measure}

A single weighted trajectory does not represent the full perturbation-specific population. The theoretical model therefore evolves a \emph{law-level finite measure} $\mu_t^g$ obtained by averaging over representative weighted trajectories. The particle system introduced later is a numerical approximation to this law-level object rather than the definition of the model itself.
...
$\mu_t^g$ & finite measure & Law-level predicted perturbation-specific population measure at time $t$.\\
$\hat\mu_t^{g,N_g}$ & random finite measure & Weighted empirical particle approximation to $\mu_t^g$.\\
$\bar\mu_t$ & finite measure & Total law-level population measure $\sum_{g\in\Gcal}\mu_t^g$.\\
$\bar M(t)$ & positive scalar & Total mass $\bar\mu_t(\Zcal)$.\\
$N_g$ & integer & Number of particles used numerically for perturbation $g$.\\
...
$\hat c_t$ & vector in $\R^L$ & Numerical approximation to $c_t$ computed from particles.\\
...
$W_t^g$ & Brownian motion & Brownian motion for a representative perturbation-$g$ trajectory.\\
$W_t^{g,i}$ & Brownian motion & Brownian motion for particle $i$ under perturbation $g$.\\
$Z_t^g$ & stochastic process & Representative latent state trajectory under perturbation $g$.\\
$Z_t^{g,i}$ & stochastic process & Latent state trajectory of particle $i$ under perturbation $g$.\\
$w_t^g$ & positive process & Representative perturbation-$g$ particle weight.\\
...
\begin{itemize}[leftmargin=2em]
    ...
    \item \emph{guide-abundance masses}: $M_g^0>0$ and $M_g^1>0$, representing normalized relative perturbation abundance at $P4$ and $P60$ on a guide-abundance scale.
\end{itemize}
...
The quantities $M_g^0$ and $M_g^1$ should be read as abundance proxies on a normalized guide-abundance scale rather than as exact absolute clone sizes. The finite measures in \cref{eq:mass_scaled_measures} are therefore effective endpoint targets. If one wants to propagate mass uncertainty explicitly, one may introduce latent true masses together with an observation model for the abundance readout, but that extension is not required for the core formulation here.

\subsection{Practical support requirement}

The formulas in \cref{eq:empirical_prob_measures} assume $n_g^0,n_g^1>0$. In practice, perturbations with extremely small transcriptomic support should be filtered, pooled across sgRNAs targeting the same gene, or handled with a partial observation model that uses guide abundance without a full state-resolved endpoint distribution. The exposition below assumes sufficient single-cell support at both endpoints for the perturbations that are modeled.
...
\begin{enumerate}[label=(A\arabic*),leftmargin=2.8em]
    ...
    For each perturbation $g$, conditional on latent state and the shared tissue context, the joint law of cells carrying $g$ is invariant under permutation of cell indices. Thus, cells with the same perturbation are modeled as interchangeable particles, and the perturbation-specific population law is the sufficient population-level state.

    \item \textbf{Relative-mass interpretation.}
    The masses $M_g^0$ and $M_g^1$ are treated as guide-abundance-scale abundance proxies and are therefore comparable across perturbations after normalization, even though they are distinct from the number of sequenced single-cell transcriptomes.

    \item \textbf{Mean-field closure.}
    The effect of the tissue context on any cell is approximated through low-dimensional aggregate summaries of the joint population rather than through explicit pairwise interactions between all individual cells.

    \item \textbf{Control anchoring.}
    Control perturbations define the baseline continuation law at the coefficient level. Perturbation effects are represented as low-dimensional deviations away from that baseline rather than as fully unrelated dynamics for each perturbation.
    ...
    \item \textbf{Pooled-context approximation.}
    When replicate-specific tissue context is not explicitly modeled, the single context vector $c_t$ is interpreted as an effective screen-average environment rather than as the literal within-animal environment experienced by every cell.
\end{enumerate}
...
Many perturb-seq experiments use multiple sgRNAs for the same target. In that setting, a hierarchical decomposition is often preferable:
\begin{equation}
a_{gr} = a_g + \delta_{gr},
\qquad
\delta_{gr}\in\R^r,
...
\end{equation}
with shrinkage of $\delta_{gr}$ toward zero. For expositional simplicity, the main formulas below use a single perturbation-level embedding $a_g$.
...
This expression is the precise mathematical content of the mean-field coupling: the coefficient fields are allowed to depend on the current population only through the context vector $c_t$. The no-context baseline is recovered by setting
\[
c_t \equiv 0.
\]
...
\paragraph{Sample-specific extension.}
If the data include distinct mice, tumours, or experimental batches, a more faithful extension is to index populations and contexts by sample $s$, for example through $(\mu_{s,t}^g)_g$ and $c_{s,t}=\Psi_{\theta_c}(\mu_{s,t})$. The pooled version in \cref{eq:generic_context,eq:interpretable_context} should then be interpreted as an effective screen-average context.
...
Because controls satisfy $a_g=0$, \cref{eq:drift_total,eq:disp_preact,eq:growth_preact} imply that, \emph{conditional on the shared context trajectory $c_t$}, control perturbations evaluate the baseline coefficient fields
\[
\beta_b(\cdot),
\qquad
\beta_\sigma(\cdot),
...
\beta_r(\cdot).
\]
This is an exact coefficient-level anchor. It should not be confused with the stronger claim that controls evolve in a perturbation-free global environment, because the shared context $c_t$ can itself depend on the full perturbation mixture.

\subsection{Law-level representative process}

For each perturbation $g$, let $(Z_t^g,w_t^g)$ denote a representative weighted trajectory solving
\begin{equation}
\dd Z_t^g
=
b_g\big(Z_t^g,t;\mu_t\big)\,\dd t
+
\sigma_g\big(Z_t^g,t;\mu_t\big)\,\dd W_t^g,
\qquad
Z_0^g \sim p_0^g,
\label{eq:law_sde}
\end{equation}
...
=
r_g\big(Z_t^g,t;\mu_t\big),
\qquad
w_0^g=1,
\label{eq:law_weight}
\end{equation}
where $W_t^g$ is a $d$-dimensional Brownian motion.

The perturbation-specific law-level finite measure is then defined by
\begin{equation}
\int_{\Zcal} f(z)\,\mu_t^g(\dd z)
=
M_g^0\,\E\!\left[w_t^g f(Z_t^g)\right]
\qquad
\text{for every bounded measurable } f:\Zcal\to\R.
\label{eq:law_measure_def}
\end{equation}
In particular, taking $f\equiv 1$ gives the predicted terminal mass
\begin{equation}
\hat M_g^1
=
\mu_1^g(\Zcal)
=
M_g^0\,\E\!\left[w_1^g\right].
\label{eq:pred_terminal_mass}
\end{equation}
At $t=0$, \cref{eq:law_measure_def} recovers $\mu_0^g=M_g^0p_0^g$.

\subsection{Weak measure form at the law level}
...
The weighted particle system is a numerical approximation to the law-level measure $\mu_t^g$. For each perturbation $g$, let $\{(Z_t^{g,i},w_t^{g,i})\}_{i=1}^{N_g}$ solve
\begin{equation}
\dd Z_t^{g,i}
=
b_g\big(Z_t^{g,i},t;\hat\mu_t\big)\,\dd t
+
\sigma_g\big(Z_t^{g,i},t;\hat\mu_t\big)\,\dd W_t^{g,i},
\label{eq:particle_sde}
\end{equation}
\begin{equation}
\frac{\dd}{\dd t}\log w_t^{g,i}
=
r_g\big(Z_t^{g,i},t;\hat\mu_t\big),
\qquad
w_0^{g,i}=1,
...
\in\Mplus(\Zcal).
\label{eq:particle_measure}
\end{equation}
Here $\hat\mu_t=(\hat\mu_t^{g,N_g})_{g\in\Gcal}$ denotes the current family of empirical measures.

The simplest implementation takes $N_g=n_g^0$ and initializes the particles at the observed $P4$ cells,
\[
Z_0^{g,i}=x_{gi}.
\]
Then \cref{eq:particle_measure} matches the observed initial finite measure exactly at the numerical level.

\subsection{Empirical weak form and martingale fluctuation}

Because \cref{eq:particle_measure} is a random empirical measure, its dynamics are not purely deterministic. For every smooth test function $f:\Zcal\to\R$,
\begin{equation}
\dd \int_{\Zcal} f(z)\,\hat\mu_t^{g,N_g}(\dd z)
=
\left[
\int_{\Zcal}
\left(
...
+
\dd M_t^{g,f,N_g},
\label{eq:empirical_weak_form}
\end{equation}
where the martingale term is
\begin{equation}
\dd M_t^{g,f,N_g}
=
\frac{M_g^0}{N_g}
\sum_{i=1}^{N_g}
w_t^{g,i}\,\nabla f\!\left(Z_t^{g,i}\right)^\top
\sigma_g\big(Z_t^{g,i},t;\hat\mu_t\big)
\dd W_t^{g,i}.
\label{eq:martingale_term}
\end{equation}
Thus, the deterministic weak form in \cref{eq:weak_form} and the formal reaction--drift--diffusion equation in \cref{eq:fokker_planck} apply to the law-level measure $\mu_t^g$, whereas the empirical particle approximation satisfies the same drift part plus a martingale fluctuation term.

\subsection{Regularity conditions}

For well-posedness of the law-level model and its particle approximation, we assume:
\begin{enumerate}[label=(R\arabic*),leftmargin=2.8em]
    \item the outputs of the three coefficient networks are measurable in $(t,\eta)$, where $\eta$ denotes the finite-dimensional context argument, and are locally Lipschitz in $z$;
    \item the context map depends on $\mu_t$ through a bounded finite-dimensional summary and is locally Lipschitz in that summary;
...
c(z,z')=\norm{z-z'}_2^2.
\]
Because the OT loss is sensitive to latent geometry, this cost should be evaluated in a scaled or whitened latent space whenever the coordinates have strongly unequal variance. More general Mahalanobis or learned costs can also be substituted.
...
The model-level endpoint loss is
\begin{equation}
...
In computation, this loss is approximated by replacing $\mu_1^g$ with the particle measure $\hat\mu_1^{g,N_g}$:
\begin{equation}
\widehat{\mathcal{L}}_{\mathrm{end}}
=
\sum_{g\in\Gcal}
\alpha_g^{\mathrm{end}}
\,\SDunb_{\varepsilon,\tau}\!\left(\hat\mu_1^{g,N_g},\nu_1^g\right).
\label{eq:L_end_particle}
\end{equation}
...
\subsection{Model-side growth versus loss-side mass relaxation}

The reaction field $r_g$ and the unbalanced OT relaxation parameter $\tau$ both influence how terminal mass mismatch is resolved, but they play different roles.

The field $r_g$ belongs to the forward biological model: it determines how perturbation-specific mass evolves along the interval. By contrast, the KL marginal relaxation in \cref{eq:uot_def} belongs to the endpoint discrepancy: it allows imperfect agreement between predicted and observed finite measures when guide-abundance calibration is noisy, when the finite-particle approximation is imperfect, or when strict balanced transport is too rigid.

Because only a single interval is observed, these two mechanisms are only partially identifiable from endpoints alone. This is one reason to bound and regularize $r_g$ and to choose $\tau$ conservatively rather than allowing the loss to absorb all mass mismatch.
...
From only two state-resolved time points, drift, diffusion, growth, and context feedback are not equally identifiable. In practice:
\begin{itemize}[leftmargin=2em]
    ...
    \item context effects should remain low-dimensional and well constrained.
\end{itemize}
...
In numerical training, $\mathcal{L}_{\mathrm{end}}$ is replaced by its particle approximation $\widehat{\mathcal{L}}_{\mathrm{end}}$ from \cref{eq:L_end_particle}.
...
\ell=0,\dots,L-1.
\]
Using \cref{eq:particle_sde,eq:particle_weight}, a standard Euler--Maruyama update is
\begin{equation}
Z_{\ell+1}^{g,i}
=
Z_{\ell}^{g,i}
+
b_g\!\left(Z_\ell^{g,i},t_\ell;\hat\mu_{t_\ell}\right)\Delta t
+
\sigma_g\!\left(Z_\ell^{g,i},t_\ell;\hat\mu_{t_\ell}\right)\sqrt{\Delta t}\,\xi_\ell^{g,i},
\label{eq:em_state}
\end{equation}
\begin{equation}
w_{\ell+1}^{g,i}
...
w_{\ell}^{g,i}
\exp\!\left(
r_g\!\left(Z_\ell^{g,i},t_\ell;\hat\mu_{t_\ell}\right)\Delta t
\right),
\label{eq:em_weight}
\end{equation}
where
...
=
\sum_{g\in\Gcal}
\hat\mu_{t_\ell}^{g,N_g}(\Zcal)
=
\sum_{g\in\Gcal}
\frac{M_g^0}{N_g}\sum_{i=1}^{N_g} w_\ell^{g,i},
\label{eq:discrete_total_mass}
\end{equation}
\begin{equation}
\hat q_{t_\ell}
=
\frac{1}{\hat{\bar M}(t_\ell)}
\sum_{g\in\Gcal}
\frac{M_g^0}{N_g}
\sum_{i=1}^{N_g}
...
and then sets
\begin{equation}
\hat c_{t_\ell}=\Psi_{\theta_c}(\hat q_{t_\ell},\hat s_{t_\ell}).
\label{eq:discrete_c}
\end{equation}
If a different context map is used, one updates $\hat c_{t_\ell}$ according to that construction instead.

\subsection{Discrete terminal measures}

At the terminal step $t_L=1$, the particle approximation to the predicted finite measure is
\begin{equation}
\hat\mu_1^{g,N_g}
=
\frac{M_g^0}{N_g}
...
\end{equation}
The discrete unbalanced OT problem underlying \cref{eq:L_end_particle} is then built from the cost matrix
\[
...
\begin{enumerate}[label=(\arabic*),leftmargin=2.2em]
    ...
    \item choose normalized guide-abundance masses $M_g^0,M_g^1$ and, if needed, filter or pool perturbations with insufficient state support;
    ...
    \item compute the terminal empirical measures $\hat\mu_1^{g,N_g}$ and the derived summaries $\hat M_g^1$, $\hat p_1^g$, and optionally $\hat q_{g,1}$;
    \item evaluate the approximate endpoint loss \cref{eq:L_end_particle} and the regularizers \cref{eq:R_emb,eq:R_mod,eq:R_disp,eq:R_nn};
...
\mu_1^g,
\]
which jointly encodes terminal abundance and terminal state allocation. In practice, this object is accessed numerically through its particle approximation $\hat\mu_1^{g,N_g}$.
...
\end{enumerate}
starting from the same initial state distribution and the same chosen context protocol. This directly quantifies the perturbation effect relative to the control continuation law.
...
\begin{quote}
For each perturbation $g$, start from the observed $P4$ finite measure $\mu_0^g=M_g^0p_0^g$, define a law-level weighted McKean--Vlasov SDE in latent space whose drift, stochastic dispersion, and growth are modeled by three separate control-anchored neural networks conditioned on latent state, time, and low-dimensional tissue context, approximate that law numerically by a weighted particle system, and fit the resulting terminal finite measure $\mu_1^g$ to the observed $P60$ finite measure $\nu_1^g=M_g^1p_1^g$ by a single unbalanced optimal-transport discrepancy.
\end{quote}

Equivalently, the model has five layers:
\begin{enumerate}[label=(\arabic*),leftmargin=2.2em]
    \item a \emph{biological mechanism layer}, which separates movement, stochastic spread, and abundance change;
    \item a \emph{law-level SDE layer}, which defines the reaction--drift--diffusion finite-measure dynamics;
    \item a \emph{particle approximation layer}, which provides a trainable Monte Carlo approximation to the law-level dynamics;
...
This is the revised formal version of the $P4 \to P60$ control-anchored mean-field neural differential equation formulation after making the law-versus-particle distinction explicit, clarifying the role of mass proxies and unbalanced endpoint fitting, and preserving the biology-first presentation.
